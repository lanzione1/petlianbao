...
async function seed() {
  ...
  const merchants = [
    { id: 'a0000000-0000-0000-0000-000000000001', shopName: '萌宠乐园', phone: '13800138001', planType: 'professional' },
    { id: 'a0000000-0000-0000-0000-000000000002', shopName: '爱宠之家', phone: '13800138002', planType: 'basic' },
    { id: 'a0000000-0000-0000-0000-000000000003', shopName: '小可爱宠物店', phone: '13800138003', planType: 'free' },
    { id: 'a0000000-0000-0000-0000-000000000004', shopName: '宠物世界', phone: '13800138004', planType: 'enterprise' },
    { id: 'a0000000-0000-0000-0000-000000000005', shopName: '喵汪星球', phone: '13800138005', planType: 'professional' },
    { id: 'a0000000-0000-0000-0000-000000000006', shopName: '快乐宠物', phone: '13800138006', planType: 'basic' },
    { id: 'a0000000-0000-0000-0000-000000000007', shopName: '萌宠阁', phone: '13800138007', planType: 'free' },
    { id: 'a0000000-0000-0000-0000-000000000008', shopName: '宠物驿站', phone: '13800138008', planType: 'professional' },
    { id: 'a0000000-0000-0000-0000-000000000009', shopName: '爱心宠物店', phone: '13800138009', planType: 'banned' },
    { id: 'a0000000-0000-0000-0000-000000000010', shopName: '毛孩子俱乐部', phone: '13800138010', planType: 'basic' },
  ];
  ...
  for (const m of merchants) {
    ...
    `, [m.id, `seed_${m.id}`, m.shopName, m.phone, `北京市朝阳区xxx路${Math.floor(Math.random() * 100)}号`, m.planType, dates[Math.floor(Math.random() * 30)], now]);
  }
  console.log(`Created ${merchants.length} merchants`);

  // 2. 创建客户
  console.log('Creating customers...');
  const petNames = ['豆豆', '旺财', '小黑', '小白', '大黄', '毛毛', '球球', '咪咪', '欢欢', '乐乐', '多多', '花花', '虎子', '妞妞', '嘟嘟'];
  const breeds = ['金毛', '泰迪', '柯基', '哈士奇', '布偶猫', '英短', '比熊', '博美', '柴犬', '橘猫'];
  let customerId = 1;
  for (const merchant of merchants.filter(m => m.planType !== 'banned')) {
    ...
    for (let i = 0; i < customerCount; i++) {
      ...
      `, [id, merchant.id, petName, breed, `139${Math.floor(Math.random() * 100000000).toString().padStart(8, '0')}`, totalSpent, Math.floor(Math.random() * 20) + 1, dates[Math.floor(Math.random() * 30)]]);
      customerId++;
    }
  }
  ...
  for (const merchant of merchants.filter(m => m.planType !== 'banned')) {
    for (const svc of services) {
      ...
      await AppDataSource.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
        ON CONFLICT (id) DO NOTHING
      `, [id, merchant.id, svc.name, svc.price, 30 + Math.floor(Math.random() * 60), dates[Math.floor(Math.random() * 30)]]);
      serviceId++;
    }
  }
  ...
  const statuses = ['pending', 'completed', 'paid', 'cancelled_by_merchant', 'cancelled_by_customer'];
  let appointmentId = 1;
  
  const existingCustomers = await AppDataSource.query(`SELECT id, merchant_id FROM customers LIMIT 200`);
  const existingServices = await AppDataSource.query(`SELECT id, merchant_id FROM services LIMIT 100`);
  ...
  for (const merchant of merchants.filter(m => m.planType !== 'banned')) {
    ...
    for (let i = 0; i < appointmentCount; i++) {
      ...
      const customerId = merchantCustomers.length > 0 
        ? merchantCustomers[Math.floor(Math.random() * merchantCustomers.length)].id 
        : null;
      const serviceId = merchantServices.length > 0 
      ...
      await AppDataSource.query(`
        ...
        ON CONFLICT (id) DO NOTHING
      `, [id, merchant.id, customerId, serviceId, date, status, dates[Math.floor(Math.random() * 30)]]);
      appointmentId++;
    }
  }
  ...
  for (const merchant of merchants.filter(m => m.planType !== 'banned')) {
    ...
    for (let i = 0; i < transactionCount; i++) {
      ...
      await AppDataSource.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        ON CONFLICT (id) DO NOTHING
      `, [id, merchant.id, customerId, JSON.stringify([{name: '服务', price: amount, quantity: 1}]), amount, paymentMethod, date]);
      transactionId++;
    }
  }
  ...
  for (const merchant of merchants.filter(m => m.planType !== 'banned')) {
    ...
    for (let i = 0; i < withdrawalCount; i++) {
      ...
      await AppDataSource.query(`
        ...
        ON CONFLICT (id) DO NOTHING
      `, [id, merchant.id, amount, '中国工商银行', `6222${Math.floor(Math.random() * 10000000000000).toString().padStart(14, '0')}`, merchant.shopName, status, date, now]);
      withdrawalId++;
    }
  }
  ...
  for (const merchant of merchants.filter(m => m.planType !== 'banned')) {
    ...
    for (let i = 0; i < mediaCount; i++) {
      ...
      await AppDataSource.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        ON CONFLICT (id) DO NOTHING
      `, [id, merchant.id, type, `${type}_${i + 1}.${type === 'image' ? 'jpg' : type === 'video' ? 'mp4' : 'pdf'}`, `https://example.com/${type}/${id}`, size, dates[Math.floor(Math.random() * 30)]]);
      mediaId++;
    }
  }
  ...
  if (adminId) {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      await AppDataSource.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        ON CONFLICT (id) DO NOTHING
      `, [id, adminId, action, 'merchant', merchant.id, JSON.stringify({ shopName: merchant.shopName }), dates[Math.floor(Math.random() * 30)]]);
      logId++;
    }
