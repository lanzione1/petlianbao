...
export class DevController {
  ...
  async seedData(@Request() req) {
    ...
    // 创建测试客户（以人为主）
    const customers = [
      { petName: '张三', petBreed: '', phone: '13800000001', merchantId, totalSpent: 2580, visitCount: 12, lastVisitAt: new Date() },
      { petName: '李四', petBreed: '', phone: '13800000002', merchantId, totalSpent: 1850, visitCount: 8, lastVisitAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) },
      { petName: '王五', petBreed: '', phone: '13800000003', merchantId, totalSpent: 3200, visitCount: 15, lastVisitAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) },
      { petName: '赵六', petBreed: '', phone: '13800000004', merchantId, totalSpent: 980, visitCount: 5, lastVisitAt: new Date(Date.now() - 45 * 24 * 60 * 60 * 1000) },
      { petName: '钱七', petBreed: '', phone: '13800000005', merchantId, totalSpent: 1560, visitCount: 7, lastVisitAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) },
    ];

    const createdCustomers = await customerRepo.save(
      customers.map(c => customerRepo.create(c))
    );

    // 为每个客户创建宠物
    const petsData = [
      // 张三的宠物
      { customerId: createdCustomers[0].id, name: '旺财', species: 'dog' as const, breed: '金毛', weight: 25, birthday: new Date('2020-03-15'), nextVaccineDate: new Date('2026-05-01'), nextDewormDate: new Date('2026-04-01') },
      { customerId: createdCustomers[0].id, name: '豆豆', species: 'cat' as const, breed: '英短', weight: 4.5, birthday: new Date('2021-06-20'), nextVaccineDate: new Date('2026-08-15'), nextDewormDate: new Date('2026-06-01') },
      // 李四的宠物
      { customerId: createdCustomers[1].id, name: '小白', species: 'dog' as const, breed: '萨摩耶', weight: 22, birthday: new Date('2019-11-10'), nextVaccineDate: new Date('2026-03-20'), nextDewormDate: new Date('2026-03-01') },
      // 王五的宠物
      { customerId: createdCustomers[2].id, name: '布丁', species: 'dog' as const, breed: '柯基', weight: 12, birthday: new Date('2022-02-14'), nextVaccineDate: new Date('2026-04-10'), nextDewormDate: new Date('2026-03-15') },
      { customerId: createdCustomers[2].id, name: '米粒', species: 'cat' as const, breed: '布偶', weight: 5, birthday: new Date('2023-01-01'), nextVaccineDate: new Date('2026-07-01'), nextDewormDate: new Date('2026-05-01') },
      // 赵六的宠物
      { customerId: createdCustomers[3].id, name: '哈利', species: 'dog' as const, breed: '哈士奇', weight: 20, birthday: new Date('2021-08-25'), nextVaccineDate: new Date('2026-02-28'), nextDewormDate: new Date('2026-02-20') },
      // 钱七的宠物
      { customerId: createdCustomers[4].id, name: '皮皮', species: 'dog' as const, breed: '泰迪', weight: 8, birthday: new Date('2020-12-05'), nextVaccineDate: new Date('2026-06-01'), nextDewormDate: new Date('2026-04-15') },
    ];
    ...
    const createdServices = await serviceRepo.save(
      services.map(s => serviceRepo.create(s))
    );
  ...
  async seedH5Data(@Request() req) {
    ...
    // 创建测试H5客户
    const h5Customers = [
      { merchantId, openid: 'test_openid_001', nickname: '测试用户1', phone: '13900000001', avatar: '' },
      { merchantId, openid: 'test_openid_002', nickname: '测试用户2', phone: '13900000002', avatar: '' },
      { merchantId, openid: 'test_openid_003', nickname: '测试用户3', phone: '13900000003', avatar: '' },
    ];
    ...
    // 为每个客户创建宠物
    const h5PetsData = [
      { customerId: createdH5Customers[0].id, name: '小黄', species: 'dog' as const, breed: '金毛', gender: 'male' as const, weight: 25 },
      { customerId: createdH5Customers[0].id, name: '小花', species: 'cat' as const, breed: '英短', gender: 'female' as const, weight: 4.5 },
      { customerId: createdH5Customers[1].id, name: '大白', species: 'dog' as const, breed: '萨摩耶', gender: 'male' as const, weight: 22 },
      { customerId: createdH5Customers[2].id, name: '小黑', species: 'dog' as const, breed: '拉布拉多', gender: 'male' as const, weight: 28 },
    ];

    const createdH5Pets = await h5PetRepo.save(
      h5PetsData.map(p => h5PetRepo.create(p))
    );
