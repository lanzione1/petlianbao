...
export class CustomersService {
  ...
  async findAll(
    ...
    if (search) {
      queryBuilder.andWhere('(customer.petName LIKE :search OR customer.phone LIKE :search)', {
        search: `%${search}%`,
      });
    }
    ...
    if (customerIds.length > 0) {
      const petCounts = await this.dataSource
        .getRepository(Pet)
  ...
  async remove(merchantId: string, id: string): Promise<void> {
    ...
    console.log(`[Delete Customer] Found customer:`, {
      id: customer.id,
    ...
    console.log(
      `[Delete Customer] Pending appointments (customerId): ${pendingAppointments.length}`,
    );
    ...
    if (customer.openid) {
      ...
      try {
        ...
        if (h5Customer) {
          ...
          console.log(
            `[Delete Customer] Pending appointments (h5CustomerId): ${h5PendingAppointments.length}`,
          );
        }
      } catch (error) {
