...
export const adminApi = {
  ...
  // 商家统计
  getMerchantStats: () => api.get('/admin/merchants/stats'),

  // 商家详情
  getMerchant: (id: string) => api.get(`/admin/merchants/${id}`),

  // 审核通过
  approveMerchant: (id: string) => api.put(`/admin/merchants/${id}/approve`),

  // 禁用商家
  banMerchant: (id: string, reason: string) => api.put(`/admin/merchants/${id}/ban`, { reason }),

  // 解禁商家
  unbanMerchant: (id: string) => api.put(`/admin/merchants/${id}/unban`),
  ...
  // 平台趋势
  getPlatformTrend: (days?: number) => api.get('/admin/platform/trend', { params: { days } }),
