...
export const h5AuthApi = {
  ...
  wechatCallback(code: string, state: string) {
    return api.get<{ success: boolean; data: { customer: H5Customer; token: string } }>('/h5/auth/wechat/callback', {
      params: { code, state }
    })
  },
...
export const h5AppointmentApi = {
  ...
  reschedule(id: string, data: { proposedTime: string; notes?: string }) {
    return api.put<{ success: boolean; data: H5Appointment }>(`/h5/appointments/${id}/reschedule`, data)
  },
  ...
  accept(id: string, notes?: string) {
    return api.put<{ success: boolean; data: H5Appointment }>(`/h5/appointments/${id}/accept`, { notes })
  },
  ...
  cancel(id: string, reason?: string) {
    return api.put<{ success: boolean; message: string }>(`/h5/appointments/${id}/cancel`, { reason })
  }
}
